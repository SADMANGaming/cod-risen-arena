...
#define GAME_SYSCMD_ADDR 0x008E3710
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nCmdShow) {
    hInst = hInstance;

    char* game_sys_cmdline = reinterpret_cast<char*>(GAME_SYSCMD_ADDR);

    if (lpCmdLine && lpCmdLine[0]) {
        strncpy(game_sys_cmdline, lpCmdLine, 4096 - 1);
        game_sys_cmdline[4095] = '\0';
    } else {
        game_sys_cmdline[0] = '\0';
    }

    // Debug: show what we copied
    MessageBoxA(NULL, game_sys_cmdline, "sys_cmdline", MB_OK);
    ...
    return mainx(hInstance, hPrevInstance, game_sys_cmdline, nCmdShow);
}